...
function doPost(e) {
  try {
    ...
    if (data.action === "submit") {
      ...
      var cost = (emailCount === 0 || existingRow > 0) ? 15 : 10;
      var row = [
        new Date(),
        data.email,
        data.firstName || "",
...
function doGet(e) {
  try {
    ...
    // Chat — get messages
    if (mode === "chat") {
      var chatSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Chat");
      if (!chatSheet) {
        return ContentService.createTextOutput(JSON.stringify({
      ...
      var messages = [];
      for (var ci = 1; ci < chatData.length; ci++) {
        if (!chatData[ci][1]) continue;
        messages.push({
          timestamp: chatData[ci][0] ? new Date(chatData[ci][0]).toISOString() : "",
          name: chatData[ci][1] || "",
          message: chatData[ci][2] || "",
        });
      }
    ...
    if (mode === "chatpost") {
      ...
      if (!chatSheet) {
        ...
        chatSheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet("Chat");
        chatSheet.appendRow(["Timestamp", "Name", "Message"]);
      }
      var p = e.parameter;
      var chatName = p.name || "Anonymous";
      var chatMsg = p.message || "";
      if (chatMsg.length > 280) chatMsg = chatMsg.substring(0, 280);
      if (chatMsg.trim()) {
        chatSheet.appendRow([new Date(), chatName, chatMsg.trim()]);
      }
      return ContentService.createTextOutput(JSON.stringify({
    ...
    for (var i = 1; i < data.length; i++) {
      var row = data[i];
      if (!row[1]) continue;
      // Calculate cost: first entry per email = $15, second = $10
      var entryEmail = row[1];
      if (!emailCounts[entryEmail]) emailCounts[entryEmail] = 0;
      emailCounts[entryEmail]++;
      var entryCost = emailCounts[entryEmail] === 1 ? 15 : 10;
      totalPot += entryCost;
      entries.push({
        ...
        winningScore: row[11],
        family: (row[14] || "").toString().toUpperCase() === "Y",
      });
    }
    ...
    return ContentService.createTextOutput(JSON.stringify({
      ...
      totalPot: totalPot,
      payouts: {
        first: Math.round(totalPot * 0.60),
        second: Math.round(totalPot * 0.25),
        third: Math.round(totalPot * 0.15),
      },
    })).setMimeType(ContentService.MimeType.JSON);
